...
def problem_1_2(n):
    h = math.pi / (n)
    f = lambda x: -math.sin(x)
    X = np.linspace(0, math.pi, num = n+1)[1:-1]
    fi = np.array([f(i) for i in X])
    realsol = np.array([math.sin(i) for i in X])
...
def problem_1_3(n):
    ...
    fi = np.array([f(i) for i in X[1:-1]])
    realsol = np.array([math.sin(i) for i in X])

    ab = np.array([[0]+[-1 for i in range(n-2)],[2 for i in range(n-1)],[-1 for i in range(n-2)]+[0]])
...
def problem_1_4(n):
    ...
    X = np.linspace(0,math.pi, num = n+1)
...
def problem_2_1(n):
    ...
    fi = [f(x*h,y*h-math.pi/2) for x in range(1,n) for y in range(1,n)]
    realsol = [math.cos(x*h)*math.sin(y*h-math.pi/2) for x in range(1,n) for y in range(1,n)]
    ...
    while count1 < 10000 and error1 > 0.0001:
        count1 += 1
        jacobsol, error1 = calcjacobi(matL, invD, matU, jacobsol, fi, realsol,h)
        error1 = h*error1
    ...
    while count2 < 10000 and error2 > 0.0001:
        count2 += 1
        GSsol, error2 = calcGS(invLD, matU, GSsol, fi, realsol,h)
        error2 = h*error2
    ...
    realsol = realsol.reshape((n-1,n-1))
...
def problem_2_2(n):
    ...
    fi = np.array([f(x,y) for x in X for y in Y])

    realsol = np.array([math.sin(x)*math.cos(y) for x in X for y in Y])
    ...
    while count1 < 10000 and error1 > 0.0001:
        ...
        error1 = np.sum(h**2*(jacobsol - realsol)**2)
    ...
    while count2 < 10000 and error2 > 0.0001:
        ...
        GSsol = np.dot(-matU,GSsol) - h**2*fi
        GSsol = invLD.dot(GSsol)
        error2 = np.sum((GSsol - realsol)**2*h**2)
...
def problem_2_3(n):
    ...
    Y = np.linspace(y0, yn, n+1)[1:-1]
    f = lambda x,y: -2*math.sin(x)*math.cos(y)
    fi = np.array([f(x*h,y*h) for x in range(n-1) for y in range(n-1)])
    realsol = np.array([math.sin(x*h)*math.cos(y*h) for x in range(n-1) for y in range(n-1)])
    ...
    realsol = realsol.reshape((n-1,n-1))
    X = np.array([h*i for i in range(1,n)])
    Y = np.array([h*i for i in range(1,n)])
    ...
    ay = plt.subplot(2,1,2, projection = '3d')
    ay.plot_surface(XX,YY,realsol)
    
    plt.show()

    error = np.sum((sol-realsol)**2*h**2)
    print('Scipy로 계산한 결과 error가 {}인 해를 얻었다.'.format(error))
...
def problem_2_4(n):
    jacobsol, GSsol, realsol = problem_2_2(n)   ##problem_2_1과 problem_2_2 모두 적용 가능
    h = math.pi / n
